...
class VideoService():
    ...
    def draw_thing(self, thing):
        """Draws the given thing's text on the screen.

        Args:
            thing (thing): The thing to draw.
        """ 
        text = thing.get_text()
        x = thing.get_position().get_x()
        y = thing.get_position().get_y()
        font_size = thing.get_font_size()
        color = thing.get_color().to_tuple()
        pyray.draw_text(text, x, y, font_size, color)
        
    def draw_things(self, things):
        """Draws the text for the given list of things on the screen.

        Args:
            things (list): A list of things to draw.
        """ 
        for thing in things:
